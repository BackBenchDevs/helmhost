//! Unknown server message + DesktopSize pseudo-encoding.
...
use helmhost_rfb::messages::{
    CLIENT_SET_ENCODINGS, CLIENT_SET_PIXEL_FORMAT, ENC_DESKTOP_SIZE, MSG_FRAMEBUFFER_UPDATE,
};
...
async fn unknown_server_message_emits_error() {
    ...
    let server = tokio::spawn(async move {
        let (mut stream, _) = listener.accept().await.unwrap();
        handshake_upto_fb_req(&mut stream).await;
        stream.write_all(&[99u8]).await.unwrap(); // unknown type
        let mut buf = [0u8; 32];
        let _ = timeout(Duration::from_millis(300), stream.read(&mut buf)).await;
    });
